...
BEGIN
  ...
  -- Action logs for Task 1 (Profiling)
  INSERT INTO action_logs (user_id, task_id, title, description, status) VALUES
  (test_user_id, task1_id, 'Set up New Relic APM', 'Configured New Relic for all production services and set up custom dashboards', 'on_track');

  INSERT INTO action_logs (user_id, task_id, title, description, status) VALUES
  (test_user_id, task1_id, 'Analyzed top 20 slowest endpoints', 'Identified /api/products/search and /api/orders/history as the main bottlenecks, averaging 1.2s response time', 'on_track');

  INSERT INTO action_logs (user_id, task_id, title, description, status) VALUES
  (test_user_id, task1_id, 'Documented findings in Confluence', 'Created detailed report with metrics, graphs, and recommendations for the team', 'on_track');
  ...
  INSERT INTO action_logs (user_id, task_id, title, description, status) VALUES
  (test_user_id, task2_id, 'Developed Redis caching layer in staging environment', 'Built and tested caching logic for product and user data. Seeing 60% reduction in DB queries in staging.', 'on_track');

  -- Action logs for Task 3 (Database optimization)
  INSERT INTO action_logs (user_id, task_id, title, description, status) VALUES
  (test_user_id, task3_id, 'Added composite index on orders table', 'Created index on (user_id, created_at) which reduced query time from 800ms to 120ms', 'on_track');

  INSERT INTO action_logs (user_id, task_id, title, description, status) VALUES
  (test_user_id, task3_id, 'Optimized product search query with full-text search', 'Replaced LIKE queries with PostgreSQL full-text search. Testing shows 70% improvement.', 'on_track');

  -- Action logs for Task 4 (Architecture blueprint)
  INSERT INTO action_logs (user_id, task_id, title, description, status) VALUES
  (test_user_id, task4_id, 'Facilitated 3-day architecture workshop', 'Led cross-functional workshop with 12 engineers to define service boundaries', 'on_track');

  INSERT INTO action_logs (user_id, task_id, title, description, status) VALUES
  (test_user_id, task4_id, 'Created C4 architecture diagrams', 'Documented the target architecture using C4 model - shared with engineering leadership', 'on_track');

  -- Action logs for Task 5 (Auth service)
  INSERT INTO action_logs (user_id, task_id, title, description, status) VALUES
  (test_user_id, task5_id, 'Built authentication service with JWT support', 'Implemented OAuth 2.0 and JWT token management in new auth microservice', 'on_track');

  INSERT INTO action_logs (user_id, task_id, title, description, status) VALUES
  (test_user_id, task5_id, 'Deployed auth service to staging', 'Successfully deployed and running integration tests. 95% test coverage achieved.', 'at_risk');

  -- Action logs for Task 6 (Mentorship)
  INSERT INTO action_logs (user_id, task_id, title, description, status) VALUES
  (test_user_id, task6_id, 'Assigned 3 junior engineers as mentees', 'Now mentoring Sarah, James, and Priya on backend development and system design', 'on_track');

  INSERT INTO action_logs (user_id, task_id, title, description, status) VALUES
  (test_user_id, task6_id, 'Created mentorship framework document', 'Developed a 12-week structured mentorship plan with learning objectives and milestones', 'on_track');
