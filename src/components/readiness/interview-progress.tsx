...
export function InterviewProgress({
  ...
  const stateOf = (key: string): 'covered' | 'noted' | 'current' | 'pending' => {
    // The active topic wins: while the coach is still probing a competency it
    // reads as "current" even if it already has partial evidence, so the user can
    // always see which one is in play. It flips to covered/noted once focus moves.
    if (key === focusKey) return 'current';
