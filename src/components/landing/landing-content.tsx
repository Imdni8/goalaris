...
interface LandingContentProps {
  latestRelease: { slug: string; badgeLabel: string } | null;
}

export default function LandingContent({ latestRelease }: LandingContentProps) {
  ...
  const badgeLabel = latestRelease?.badgeLabel ?? 'Beta launched';
  const badgeHref = latestRelease ? `/releases#${latestRelease.slug}` : null;
        ...
        <section className="py-24 pb-20 text-center h-[85vh] flex flex-col justify-center">
          <div className="container mx-auto px-6 max-w-[1400px]">
            {badgeHref ? (
              <Link
                href={badgeHref}
                className="group inline-flex items-center gap-2 px-4 py-2 bg-[var(--color-primary-light)] border border-[var(--color-primary-medium)] rounded-full text-[0.8rem] font-medium text-[var(--color-primary)] mb-6 transition-all duration-200 hover:bg-[var(--color-primary-medium)] hover:-translate-y-0.5 hover:shadow-[0_4px_12px_rgba(59,130,246,0.2)]"
      ...
      <footer className="py-8 text-center text-[0.85rem] text-[var(--color-text-soft)]">
        <div className="container mx-auto px-6 max-w-[1100px]">
          <div className="flex flex-col sm:flex-row items-center justify-center gap-2 sm:gap-4">
            ...
            >
              {latestRelease ? `${latestRelease.badgeLabel} · Changelog` : 'Changelog'}
            </Link>
          </div>
        </div>
      </footer>
